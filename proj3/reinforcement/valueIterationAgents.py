...
class ValueIterationAgent(ValueEstimationAgent):
        ...
        for _ in range(self.iterations):
            for state in self.mdp.getStates():
                if not self.mdp.isTerminal(state):
                    updatedValues[state] = max([self.computeQValueFromValues(state, action) 
                                                for action in self.mdp.getPossibleActions(state)])
            self.values.update(updatedValues)
    ...
    def computeActionFromValues(self, state):
        ...
        if self.mdp.isTerminal(state):
            return None
        qValueActionPairs = [[self.computeQValueFromValues(state, action), action] for action in self.mdp.getPossibleActions(state)]
        maxQValue, maxAction = max(qValueActionPairs)
        return maxAction
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
    ...
    def runValueIteration(self):
        ...
        for state in self.mdp.getStates():
            if not self.mdp.isTerminal(state):
                predecessors[state] = self.findPredecessor(state)
                diff = abs(self.values[state] - self.maxQValue(state))
    ...
    def findPredecessor(self, state):
        predecessors = set()
        for preState in self.mdp.getStates():
            for action in self.mdp.getPossibleActions(preState):
                for nextState, prob in self.mdp.getTransitionStatesAndProbs(preState, action):
                    if nextState == state and prob != 0:
                        predecessors.add(preState)
        return predecessors
